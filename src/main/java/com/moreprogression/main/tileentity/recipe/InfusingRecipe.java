...
public class InfusingRecipe implements IRecipe<IInventory> {
	...
	private final int infuseTime;
	
	public static final IRecipeType<InfusingRecipe> INFUSING = IRecipeType.register("moreprogression:infusing");
	
	public InfusingRecipe(ResourceLocation id, String group, Ingredient ingredient, ItemStack result, ItemStack secondResult, float experience, int infuseTime) {
		this.id = id;
		this.group = group;
	...
	public IRecipeSerializer<?> getSerializer() {
		return ProgressionRecipeSerializers.INFUSING;
	}

	@Override
	public ItemStack getIcon() {
		return new ItemStack(ProgressionBlocks.ender_infuser);
	}
	...
	public static class Serializer extends net.minecraftforge.registries.ForgeRegistryEntry<IRecipeSerializer<?>>  implements IRecipeSerializer<InfusingRecipe> {
		...
		public InfusingRecipe read(ResourceLocation recipeId, JsonObject json) {
			...
			int i = JSONUtils.getInt(json, "infusetime", 200);
			return new InfusingRecipe(recipeId, s, ingredient, result, secondResult, f, i);
		}
		...
		public void write(PacketBuffer buffer, InfusingRecipe recipe) {
			...
			buffer.writeVarInt(recipe.infuseTime);
		}
