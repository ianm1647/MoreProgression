...
public class CherryTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
   private static final BlockState CHERRY_LOG = ProgressionBlocks.cherry_log.getDefaultState();
   private static final BlockState CHERRY_LEAVES = ProgressionBlocks.cherry_leaves.getDefaultState();
   protected final int minTreeHeight;
   private final boolean vinesGrow;
   private final BlockState field_76532_c;
   private final BlockState field_76530_d;

   public CherryTreeFeature(Function<Dynamic<?>, ? extends NoFeatureConfig> func, boolean bool) {
      this(func, bool, 4, CHERRY_LOG, CHERRY_LEAVES, false);
   }

   public CherryTreeFeature(Function<Dynamic<?>, ? extends NoFeatureConfig> func, boolean bool1, int int1, BlockState state1, BlockState state2, boolean bool2) {
      super(func, bool1);
      this.minTreeHeight = int1;
      this.field_76532_c = state1;
      this.field_76530_d = state2;
...
public boolean place(Set<BlockPos> changedBlocks, IWorldGenerationReader worldIn, Random rand, BlockPos position, MutableBoundingBox box) {
         ...
         if (!flag) {
                  ...
                  for(int l1 = position.getZ() - j4; l1 <= position.getZ() + j4; ++l1) {
                     ...
                     if (Math.abs(k1) != j4 || Math.abs(i2) != j4 || rand.nextInt(2) != 0 && l3 != 0) {
                        ...
                        if (isAirOrLeaves(worldIn, blockpos) || func_214576_j(worldIn, blockpos)) {
                           this.setLogState(changedBlocks, worldIn, blockpos, this.field_76530_d, box);
                        }
                     }
                  }
               }
            ...
            for(int i3 = 0; i3 < i; ++i3) {
               if (isAirOrLeaves(worldIn, position.up(i3)) || func_214576_j(worldIn, position.up(i3))) {
                  this.setLogState(changedBlocks, worldIn, position.up(i3), this.field_76532_c, box);
   ...
   protected int func_208534_a(Random rand) {
